...
import sys
import time
from gtk._gtk import combo_box_new_text
...
class FileDiff(melddoc.MeldDoc, gnomeglade.Component):
    ...
    def __init__(self, prefs, num_panes):
        ...
        actions = (
            ...
            ("MakeReport", None, _("Add review line"), "<control>K", _("Make review line"), self.make_report),
        )
    ...
    def make_report(self, widget):
        current_line = self.cursor.line
        self.window = gtk.Window(gtk.WINDOW_TOPLEVEL)
        #combo init
        self.combo_box = gtk.combo_box_new_text()
        self.combo_box.append_text('A')
        self.combo_box.append_text('B')
        self.combo_box.append_text('C')
        self.combo_box.append_text('-')
        self.combo_box.set_active(0)
        self.box1 = gtk.VBox(True, 2)
        self.line_number = gtk.Entry(0)
        self.filename = gtk.Entry(0)
        self.description = gtk.Entry(0)
        self.window.add(self.box1)
        self.append_btn = gtk.Button("Append")
        self.line_number.set_text(str(current_line))
        self.filename.set_text(os.path.basename(self._get_pane_label(1)))

        self.append_btn.connect("clicked", self.callback)

        self.box1.pack_start(self.combo_box, True, True, 0)
        self.box1.pack_start(self.line_number, True, True, 0)
        self.box1.pack_start(self.filename, True, True, 0)
        self.box1.pack_start(self.description, True, True, 0)
        self.box1.pack_start(self.append_btn, True, True, 0)
    ...
    def callback(self, widget):
        txt = self.filename.get_text() + "," + (self.line_number.get_text()) + "," + self.combo_box.get_active_text() + "," + self.description.get_text() + ","
        file = open(os.path.expanduser("~")+'/review_report.txt', 'a+')
        file.write(txt+"\n")
        self.window.hide()
